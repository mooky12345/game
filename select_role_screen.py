...
from P_icon import Player_number_icon
...
class role_screen():
    ...
    #         cnt+=1
    def select_complete_animation(self,one,two,three=None):
            self.bg=background_test(image="background/vs.png")
            self.surf.fill((0,0,0))
            self.surf.blit(self.bg.surf,(0,0))
            self.surf.blit(one.surf,(1500-self.dx-two.w*two.x_size,400))
            self.surf.blit(self.test_1.surf,(1500-self.dx-two.w*two.x_size,400))
    ...
    def update(self):
        ...
        arc = pygame.Surface((50,50))
        RED=(255,0,0)
        pygame.draw.arc(self.surf, RED, (arc.get_rect()), 0, math.radians(180))
        # self.place_role_image_pos()
